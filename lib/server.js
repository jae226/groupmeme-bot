...
class Server {
     ...
     */
    static postResponse() {
        console.log('postResponse. Got post: ' + this.req)

        // The message data from GroupMe
        const requestMessage = JSON.parse(this.req.chunks[0]);

        console.log('postResponse. Successfully parsed message: ' + requestMessage)

        this.res.writeHead(200);
        this.res.end();

        console.log('postResponse. Sending response: ' + this.res)
