...
class Server {
     ...
     */
    static postResponse() {
        console.log('Server.postResponse. Got post: ' + this.req);

        // The message data from GroupMe
        const requestMessage = JSON.parse(this.req.chunks[0]);

        console.log('Server.postResponse. Successfully parsed message: ' + requestMessage);

        this.res.writeHead(200);
        this.res.end();

        console.log('Server.postResponse. Sending response: ' + this.res);

        // Give the message data to the bot
        Bot.onMessage(requestMessage);
    };
};
